...
class BluetoothService extends ChangeNotifier {
  ...
  bool get isConnected    => Platform.isAndroid ? (_connection?.isConnected ?? false) : (_serialPort?.isOpen ?? false);
  ...
  Future<void> connectTo(BluetoothDevice device) async {
    ...
    if (!Platform.isAndroid) {
      try {
        _serialPort = SerialPort(device.address);
        if (_serialPort!.openReadWrite()) {
          // The HC-05 default baud is 9600
          _serialPort!.config.baudRate = 9600;
          _connectedDeviceName = device.name ?? device.address;
          _addLog('Connected to Windows COM Port $_connectedDeviceName', LogType.system);
          
          _serialReader = SerialPortReader(_serialPort!);
          _inputSub = _serialReader!.stream.listen((Uint8List chunk) {
            _flushTimer?.cancel();
            _flushTimer = Timer(const Duration(milliseconds: 80), _flushBuffer);
            _rxBuf.write(utf8.decode(chunk, allowMalformed: true));
            _processLines();
          }, onError: (e) {
            _addLog('COM RX error: $e', LogType.error);
          });
          notifyListeners();
        } else {
          _addLog('Failed to open ${device.address}. It might be in use.', LogType.error);
        }
      } catch (e) {
        _addLog('COM Connection Error: $e', LogType.error);
      }
      return;
    }
    ...
    try {
      if (Platform.isAndroid) {
        ...
        if (_serialPort?.isOpen == true) {
          _serialPort!.write(Uint8List.fromList(utf8.encode(cmd)));
        }
      }
